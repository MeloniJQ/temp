...
export function useRealtimeData(meterId: string, initialHistoryLength: number = 24) {
  const [meterData, setMeterData] = useState<MeterData>(() => {
    ...
    return {
      ...
      dailyConsumption: Math.round(
        history.slice(-24).reduce((sum, h) => sum + h.energy, 0) * 100,
      ) / 100,
    ...
    async function refreshData() {
      try {
        ...
        const dailyConsumption = last24Entries.length > 0
          ? Math.round(last24Entries.reduce((sum, row) => sum + row.energy, 0) * 100,) / 100
        ...
        setAlerts(
          alertsData.alerts.map((alert) => ({
            id: String(alert.id),
            title: alert.level ? String(alert.level).toUpperCase() : 'Alert',
        ...
        setNotifications(insightsData.notifications ?? [])
  ...
  useEffect(() => {
    ...
    async function refreshFakeData() {
      try {
    ...
    refreshFakeData()
    const intervalId = window.setInterval(refreshFakeData, 2000)
  ...
  return { meterData, alerts, insights, notifications, backendConnected, isLoading }
}
